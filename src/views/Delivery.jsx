...
function Row(props) {
  ...
  const handleUpdate = (uid, status) => {
    if (!uid) return;

    const isOk = window.confirm("Are you sure? this can't be undone");
        ...
        <TableCell align="center" className="flex items-center justify-center">
          {row.status == "On the way" ? (
            <button
              ...
              onClick={() => handleUpdate(row.uid, "Delivered")}
            >
              Mark as Delivered
            </button>
          ) : (
            <>
              ...
              <button
                className="bg-green-700 text-white px-4 py-2 rounded-xl cursor-pointer mx-1"
                onClick={() => handleUpdate(row.uid, "On the way")}
              >
                Deliver
              </button>
            </>
...
export default function Delivery() {
  ...
  useEffect(() => {
    ...
    const whiteListStatus = ["For Deliver", "On the way"];
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", "fordelivery_data.csv");
